...
function getlikes($id){
	global $con;
	$query = "SELECT COUNT(*) as count from likes where item_id='$id'";
...
function send_email($email,$html,$subject){
	global $con;
	$row=mysqli_fetch_assoc(mysqli_query($con,"select * from email"));
...
function removeDishFromCartByid($id){
	...
	if(isset($_SESSION['USER_ID'])){
		global $con;
		$res=mysqli_query($con,"delete from dish_cart where dish_detail_id='$id' and user_id=".$_SESSION['USER_ID']);
	}else{
...
function emptyCart(){
	if(isset($_SESSION['USER_ID'])){
		global $con;
		$res=mysqli_query($con,"delete from dish_cart where user_id=".$_SESSION['USER_ID']);
	}else{
		unset($_SESSION['cart']);
	}
}
